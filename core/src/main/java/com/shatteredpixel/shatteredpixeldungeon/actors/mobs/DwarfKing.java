...
public class DwarfKing extends Mob {
	...
	protected boolean act() {
		if (phase == 1) {
			...
			if (summonsMade < 4){
				...
				if (summonsMade == 7){
					summonSubject(3, Random.Int(1) == 0 ? DKGolem.class : DKGolem.class);
				} else {
				...
				summonSubject(4, DKMaster.class);
				summonSubject(4, DKWarlock.class);
				summonSubject(4, DKWarlock.class);
				summonSubject(4, DKGolem.class);
				summonsMade = 12;
				spend(TICK);
				return true;
			} else {
	...
	private boolean summonSubject( int delay ){
		//4th summon is always golems
		if (summonsMade % 4 == 3){
			return summonSubject( delay, Random.Int(2) == 0 ? DKGolem.class : DKGolem.class );
		} else {
	...
	public static class DKMaster extends Senior {
		{
			state = HUNTING;
		}
	}
		...
		public boolean act() {
			...
			if (delay <= 0){
				...
				if (summon == DKWarlock.class){
					...
					particles.burst(BlastParticle.FACTORY, 25);
		...
		public void fx(boolean on) {
			if (on && particles == null) {
				...
				if (summon == DKWarlock.class){
					...
					particles.pour(BlastParticle.FACTORY, 0.1f);
				} else {
